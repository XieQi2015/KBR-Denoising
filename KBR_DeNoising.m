function [Emsi] = KBR_DeNoising(Nmsi,nSig,memorySaving,Omsi)
% MSI denoising based on KBRreg
%
% Input arguments:
    ...
    elseif memorySaving == 1
        ...
        for i = 1:numPart
            ...
            parfor j = 1:sizeInd(2)
                tempPatch(:,:,:,j) = KBRreg(tempPatch(:,:,:,j),1/tempSigma(j),parBSM); % Perform ITS-based tensor recovery on each FBP goup
            end
            for j = 1:sizeInd(2)
